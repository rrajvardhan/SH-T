...
TextureManager::~TextureManager()
{
  ...
  {
    IMG_Quit();
    LOG_INFO("[TextureManager] SDL_image shut down.");
  }
}
...
SDL_Texture*
TextureManager::getTexture(const std::string& id)
{
...
void
TextureManager::drawTexture(SDL_Texture* texture, SDL_Rect* src, SDL_Rect* dest)
{
  if (texture == nullptr)
  {
    LOG_ERROR("[TextureManager] Tried to draw null texture.");
...
bool
TextureManager::hasTexture(const std::string& id) const
{
  return _textures.find(id) != _textures.end();
}

void
TextureManager::unloadTexture(const std::string& id)
{
  auto it = _textures.find(id);
  if (it != _textures.end())
  {
