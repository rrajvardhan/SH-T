                         ...
                         bool                   input_source_is_clipboard)
{
  ...
  if (c < 0x20)
  {
    ...
    pass |= (c == '\n')
            && (flags & ImGuiInputTextFlags_Multiline)
                   != 0; // Note that an Enter KEY will emit \r and be ignored (we poll for KEY in
                         // InputText() code)
                   ...
                   const ImVec2&       size_arg)
{
  ...
  if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered
    ...
    ColorTooltip(
        desc_id, &col.x, flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_AlphaMask_));
...
ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count)
{
  ...
  if (ImGuiTable* table = g.CurrentTable)
    if (table->CurrentColumn != -1)
      TableEndCell(table); // This is currently safe to call multiple time. If that properly is lost
                           // we can extract the "save measurement" part of it.
        ...
        || (ms->RangeSrcPassedBy == false
            && ms->IO.RangeSrcItem
                   != ImGuiSelectionUserData_Invalid)) // Can't read storage->RangeSrcItem here ->
                                                       // we want the state at beginning of the
                                                       // scope (see tests for easy failure)
    {
               ...
               int   height_in_items)
{
  ...
  ImVec2 size(
      0.0f,
      ImTrunc(GetTextLineHeightWithSpacing() * height_in_items_f + g.Style.FramePadding.y * 2.0f));
...
static ImU32
TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, ImGuiWindow* docked_window);
static float
...
ImGui::TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window)
{
  ...
  if (!window->HasCloseButton)
    tab_flags |= ImGuiTabItemFlags_NoCloseButton; // Set _NoCloseButton immediately because it will
                                                  // be used for first-frame width calculation.
                 ...
                 ImGuiWindow*      docked_window)
{
  ...
  if (g.DragDropActive
      && !g.DragDropPayload.IsDataType(
          IMGUI_PAYLOAD_TYPE_WINDOW)) // FIXME: May be an opt-in property of the payload to disable
                                      // this
    button_flags |= ImGuiButtonFlags_PressedOnDragDropHold;
  ...
  else if (held && !tab_appearing && IsMouseDragging(0))
  {
    ...
    if (can_undock)
    {
      // We use a variable threshold to distinguish dragging tabs within a tab bar and extracting
      // them out of the tab bar
