...
// [SECTION] Test Services specific hooks (imgui_test_engine)
...
IMGUI_API void*
ImMemdup(const void* src, size_t size); // Duplicate a chunk of memory.
IMGUI_API char*
ImStrdupcpy(char*       dst,
            size_t*     p_dst_size,
...
IMGUI_API int
ImTextStrFromUtf8(ImWchar*     out_buf,
                  int          out_buf_size,
                  const char*  in_text,
...
enum ImGuiItemFlagsPrivate_
{
  ...
  = 1 << 10, // false     // Disable interactions (DOES NOT affect visuals. DO NOT mix direct use of
             // this with BeginDisabled(). See BeginDisabled()/EndDisabled() for full disable
  ...
  = 1 << 12, // false     // [BETA] Represent a mixed/indeterminate value, generally multi-selection
             // where values differ. Currently only supported by Checkbox() (later should support
...
enum ImGuiItemStatusFlags_
{
  ...
  = 1 << 0, // Mouse position is within item rectangle (does NOT mean that the window is in correct
            // z-order and can be hovered!, this is only one part of the most-common IsItemHovered
            // test)
...
enum ImGuiButtonFlagsPrivate_
{
  ...
  = 1 << 9, // return true when held into while we are drag and dropping another item (used by e.g.
  ...
  = 1
    << 18, // don't override navigation focus when activated (FIXME: this is essentially used every
  ...
  = 1 << 20, // don't set key/input owner on the initial click (note: mouse buttons are keys! often,
...
enum ImGuiSelectableFlagsPrivate_
{
  ...
  = 1 << 21, // (WIP) Auto-select when moved into. This is not exposed in public API as to handle
             // multi-select and modifiers we will need user to explicitly control focus scope. May
  ...
  = 1 << 27, // Don't set key/input owner on the initial click (note: mouse buttons are keys! often,
             // the key in question will be ImGuiKey_MouseLeft!)
};
...
enum ImGuiInputFlagsPrivate_
{
  ...
  = 1 << 3, // Repeat rate: Faster
            ...
            // - In theory ImGuiInputFlags_RepeatUntilOtherKeyPress may be a desirable default, but
            // it would break too many behavior so everything is opt-in.
...
enum ImGuiActivateFlags_
{
  ...
  = 1 << 1, // Favor activation for tweaking with arrows or gamepad (e.g. for Slider/Drag). Default
            // for Space key and if keyboard is not used.
...
enum ImGuiNavRenderCursorFlags_
{
  ...
  = 1 << 2, // Draw rectangular highlight if (g.NavId == id) even when g.NavCursorVisible == false,
            // aka even when using the mouse.
...
enum ImGuiNavMoveFlags_
{
  ...
  = 1 << 4, // Allow scoring and considering the current NavId as a move target candidate. This is
            // used when the move source is offset (e.g. pressing PageDown actually needs to send a
            // Up move request, if we are pressing PageDown from the bottom-most item we need to
            // stay in place)
  ImGuiNavMoveFlags_AlsoScoreVisibleSet
  = 1 << 5, // Store alternate result in NavMoveResultLocalVisible that only comprise elements that
...
enum ImGuiDockNodeFlagsPrivate_
{
  ...
  ImGuiDockNodeFlags_NoTabBar = 1 << 12, // Saved // Tab bar is completely unavailable. No triangle
                                         // in the corner to enable it back.
  ImGuiDockNodeFlags_HiddenTabBar
  = 1
    << 13, // Saved // Tab bar is hidden, with a triangle in the corner to show it again (NB: actual
           // tab-bar instance may be destroyed as this is only used for single-window tab bar)
  ImGuiDockNodeFlags_NoWindowMenuButton = 1 << 14, // Saved // Disable window/docking menu (that one
  ...
  // ImGuiDockNodeFlags_NoDockingOverCentralNode/ImGuiDockNodeFlags_NoDockingSplit don't have those
  // issues.
  ...
  = 1 << 22, //       // Disable this node from being docked over an empty node (e.g. DockSpace with
             //       no other windows)
...
{
  ImGuiID            ID;
  ImGuiDockNodeFlags SharedFlags; // (Write) Flags shared by all nodes of a same dockspace hierarchy
  ...
  ImGuiWindow* VisibleWindow; // Generally point to window which is ID is == SelectedTabID, but when
                              // CTRL+Tabbing this can be a different window.
  ...
  bool IsVisible : 1; // Set to false when the node is hidden (usually disabled as it has no active
                      // window)
  bool IsFocused : 1;
...
struct ImGuiViewportP : public ImGuiViewport
{
  ...
  ImGuiViewportP()
  {
    Window          = NULL;
    Idx             = -1;
    LastFrameActive = BgFgDrawListsLastFrame[0] = BgFgDrawListsLastFrame[1] = LastFocusedStampCount
        = -1;
    LastNameHash = 0;
...
enum ImGuiDebugLogFlags_
{
  ...
  ImGuiDebugLogFlags_OutputToTestEngine = 1 << 21, // Also send output to Test Services
};
...
struct ImGuiContext
{
  ...
  ImGuiViewportP*
      MouseLastHoveredViewport; // Last known viewport that was hovered by mouse (even if we are not
  ...
  int    PlatformWindowsCreatedCount; // Unique sequential creation counter (mostly for
                                      // testing/debugging)
  int ViewportFocusedStampCount; // Every time the front-most window changes, we stamp its viewport
  ...
  bool NavMousePosDirty; // When set we will update mouse position if io.ConfigNavMoveSetMousePos is
  ...
  int WantTextInputNextFrame; // Copied in EndFrame() from g.PlatformImeData.WanttextInput. Needs to
                              // be set for some backends (SDL3) to emit character inputs.
...
struct IMGUI_API ImGuiWindow
{
  ...
  ImVec2 ViewportPos; // We backup the viewport position (since the viewport may disappear or never
                      // be created if the window is inactive)
  int ViewportAllowPlatformMonitorExtend; // Reset to -1 every frame (index is guaranteed to be
                                          ...
                                          // Appearing frame of a tooltip/popup to enforce clamping
                                          // to a given monitor
  ...
  ImGuiCond
      SetWindowDockAllowFlags : 8; // store acceptable condition flags for SetNextWindowDock() use.
  ...
  float
      FontRefSize; // This is a copy of window->CalcFontSize() at the time of Begin(), trying to
                   // phase out CalcFontSize() especially as it may be called on non-current window.
  ...
  ImGuiWindow* RootWindowDockTree; // Point to ourself or first ancestor that is not a child window.
                                   // Cross through dock nodes.
  ImGuiWindow*
  ...
  // Docking
  bool DockIsActive : 1; // When docking artifacts are actually visible. When this is set, DockNode
                         // is guaranteed to be != NULL. ~~ (DockNode != NULL) &&
  ...
  ImGuiID DockId; // Backup of last valid DockNode->ID, so single window remember their dock node id
                  // even when they are not bound any more
...
enum ImGuiTabBarFlagsPrivate_
{
  ...
  = 1 << 20, // Part of a dock node [we don't use this in the master branch but it facilitate branch
...
// Fonts, drawing
IMGUI_API void
RegisterUserTexture(ImTextureData* tex); // Register external texture
IMGUI_API void
...
Initialize();
IMGUI_API void
Shutdown(); // Since 1.60 this is a _private_ function. You can call DestroyContext() to destroy the
...
IMGUI_API void
StartMouseMovingWindowOrNode(ImGuiWindow* window, ImGuiDockNode* node, bool undock);
IMGUI_API void
...
// Viewports
IMGUI_API void
TranslateWindowsInViewport(ImGuiViewportP* viewport,
...
IMGUI_API void
DestroyPlatformWindow(ImGuiViewportP* viewport);
IMGUI_API void
SetWindowViewport(ImGuiWindow* window, ImGuiViewportP* viewport);
IMGUI_API void
SetCurrentViewport(ImGuiWindow* window, ImGuiViewportP* viewport);
...
IMGUI_API void
MarkItemEdited(ImGuiID id); // Mark data associated to given item as "edited", used by
                            // IsItemDeactivatedAfterEdit() function.
IMGUI_API void
PushOverrideID(ImGuiID id); // Push given value as-is at the top of the ID stack (whereas PushID
...
IMGUI_API const ImGuiStyleVarInfo*
...
IMGUI_API void
DockContextShutdown(ImGuiContext* ctx);
IMGUI_API void
DockContextClearNodes(ImGuiContext* ctx,
                      ImGuiID       root_id,
                      bool          clear_settings_refs); // Use root_id==0 to clear all
IMGUI_API void
DockContextRebuildNodes(ImGuiContext* ctx);
IMGUI_API void
DockContextNewFrameUpdateUndocking(ImGuiContext* ctx);
IMGUI_API void
DockContextNewFrameUpdateDocking(ImGuiContext* ctx);
IMGUI_API void
DockContextEndFrame(ImGuiContext* ctx);
IMGUI_API ImGuiID
DockContextGenNodeID(ImGuiContext* ctx);
IMGUI_API void
DockContextQueueDock(ImGuiContext*  ctx,
                     ImGuiWindow*   target,
                     ...
                     bool           split_outer);
IMGUI_API void
DockContextQueueUndockWindow(ImGuiContext* ctx, ImGuiWindow* window);
IMGUI_API void
DockContextQueueUndockNode(ImGuiContext* ctx, ImGuiDockNode* node);
IMGUI_API void
...
IMGUI_API bool
DockContextCalcDropPosForDocking(ImGuiWindow*   target,
                                 ImGuiDockNode* target_node,
                                 ...
                                 bool           split_outer,
                                 ImVec2*        out_pos);
IMGUI_API ImGuiDockNode*
DockContextFindNodeByID(ImGuiContext* ctx, ImGuiID id);
IMGUI_API void
DockNodeWindowMenuHandler_Default(ImGuiContext* ctx, ImGuiDockNode* node, ImGuiTabBar* tab_bar);
IMGUI_API bool
DockNodeBeginAmendTabBar(ImGuiDockNode* node);
IMGUI_API void
...
IMGUI_API bool
GetWindowAlwaysWantOwnTabBar(ImGuiWindow* window);
IMGUI_API void
BeginDocked(ImGuiWindow* window, bool* p_open);
IMGUI_API void
BeginDockableDragDropSource(ImGuiWindow* window);
IMGUI_API void
BeginDockableDragDropTarget(ImGuiWindow* window);
IMGUI_API void
...
IMGUI_API void
DockBuilderDockWindow(const char* window_name, ImGuiID node_id);
IMGUI_API ImGuiDockNode*
DockBuilderGetNode(ImGuiID node_id);
inline ImGuiDockNode*
...
IMGUI_API void
DockBuilderRemoveNodeDockedWindows(ImGuiID node_id, bool clear_settings_refs = true);
IMGUI_API void
...
IMGUI_API void
DockBuilderSetNodePos(ImGuiID node_id, ImVec2 pos);
IMGUI_API void
DockBuilderSetNodeSize(ImGuiID node_id, ImVec2 size);
IMGUI_API ImGuiID
...
// Box-Select API
IMGUI_API bool
BeginBoxSelect(const ImRect&         scope_rect,
               ImGuiWindow*          window,
...
IMGUI_API void
MultiSelectAddSetRange(ImGuiMultiSelectTempData* ms,
                       bool                      selected,
                       int                       range_dir,
...
BeginColumns(const char*         str_id,
             int                 count,
             ImGuiOldColumnFlags flags
             = 0); // setup number of columns. use an identifier to distinguish multiple column
...
IMGUI_API void
TableAngledHeadersRowEx(ImGuiID                     row_id,
                        float                       angle,
...
IMGUI_API void
TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window);
IMGUI_API void
...
IMGUI_API void
RenderArrowDockMenu(ImDrawList* draw_list, ImVec2 p_min, float sz, ImU32 col);
IMGUI_API void
...
TextAligned(float       align_x,
            float       size_x,
            const char* fmt,
            ...); // FIXME-WIP: Works but API is likely to be reworked. This is designed for 1 item
                  // on the line. (#7024)
IMGUI_API void
...
IMGUI_API void
DebugNodeDockNode(ImGuiDockNode* node, const char* label);
IMGUI_API void
...
IMGUI_API void
DebugNodePlatformMonitor(ImGuiPlatformMonitor* monitor, const char* label, int idx);
IMGUI_API void
...
struct ImFontAtlasBuilder
{
  ...
  int RectsPackedSurface; // Number of packed pixels. Used when compacting to heuristically find the
                          // ideal texture size.
