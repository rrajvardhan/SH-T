...
inline Vector2D operator+(const Vector2D &lhs, const Vector2D &rhs) { return Vector2D(lhs.x + rhs.x, lhs.y + rhs.y); }

inline Vector2D operator-(const Vector2D &lhs, const Vector2D &rhs) { return Vector2D(lhs.x - rhs.x, lhs.y - rhs.y); }

inline Vector2D operator*(const Vector2D &vec, float scalar) { return Vector2D(vec.x * scalar, vec.y * scalar); }

inline Vector2D operator*(float scalar, const Vector2D &vec) { return vec * scalar; }
...
inline Vector2D rotateVector(const Vector2D &vec, float angleDeg) {
